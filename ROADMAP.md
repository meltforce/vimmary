...
| `[open]` | Decide what happens to `vimmary/claude-api-key` | setec | | Three of the four `vimmary/*` secrets were deleted on 2026-08-07 once each was provably redundant: `postgres-password` held the same value as `docker/vimmary/db-password` (identical SHA-256), `karakeep-api-key` was read by no code, and `mistral-api-key` had been confirmed working from `app_settings`. This one was **not** deleted, because Anthropic was deliberately left unconfigured — so setec holds the only copy of a paid key, and `setec delete` removes every version. Either store it in Settings → LLM providers and then delete it, or keep it and record that setec is its home. |
