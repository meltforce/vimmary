...
| `[open]` | Delete the four `vimmary/*` secrets from setec | setec | the migrated Mistral key is confirmed working | `vimmary/postgres-password`, `vimmary/mistral-api-key`, `vimmary/karakeep-api-key` and `vimmary/claude-api-key` are unread once this release is live. `docker/vimmary/db-password` stays — it is what `run.sh` reads to render the stack's `.env`. Deleting before the key is confirmed in the GUI removes the only copy. |
