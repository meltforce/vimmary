...
| `[open]` | Give the frontend a test runner, or decide it does not get one | `web/` | the first UI regression that a type check would not have caught | `npm run build` (`tsc -b && vite build`) is the entire frontend gate — no linter, no formatter, no tests, in CI or in the `verify` skill. The migration replaced six page components with no test to fail, and the two layout defects it introduced were found by looking at screenshots. What a runner would buy: `display.ts` is pure and testable (`groupByDay`, `dayLabel`, `statusClass`), and `useIsDesktop()` decides which of two component trees renders. What it costs: a dependency, a CI job and a habit that does not exist here yet. |
